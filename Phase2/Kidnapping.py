...
class Main_file:
    ...
    def generate_random_particle(self, map_obj, initialize):
        ...
        while count < num_of_particles:
            ...
            _x = np.random.uniform(x_min, x_max)
            _y = np.random.uniform(y_min, y_max)
            _theta = np.random.uniform(theta - np.pi/6, theta + np.pi/6)
            if map_obj.check_point_feasibility([_x, _y]):
                ...
                if initialize:
                    ...
                    count = count + 1
    def generate_random_particle_initial(self, map_obj, initialize):
        num_of_particles = self.num_of_particles
        count = 0
        particles = []
        while count < num_of_particles:
            ...
            _y = np.random.uniform(y_min, y_max)
            _theta = np.random.uniform(-np.pi, np.pi)
            _x = np.random.normal(x, 0.02); _y = np.random.normal(y, 0.02); _theta = np.random.normal(theta, 0.01)
            if map_obj.check_point_feasibility([_x, _y]):
                rand_point = [_x, _y, _theta]
                if initialize:
                    return rand_point
        ...
        return particles
    def generate_random_particle_kidnapped(self, map_obj, initialize):
     num_of_particles = self.num_of_particles
     count = 0
     particles = []
     _rand_point = []
     while True:
         ...
         _theta = np.random.uniform(-np.pi, np.pi)
         if map_obj.check_point_feasibility([_x, _y]):
             _rand_point = [_x, _y, _theta]
             break
     while count < num_of_particles:
            _x = np.random.normal(_rand_point[0], 0.02); _y = np.random.normal(_rand_point[1], 0.02); _theta = np.random.normal(-np.pi/6, np.pi/6)
            if map_obj.check_point_feasibility([_x, _y]):
                rand_point = [_x, _y, _theta]
                if initialize:
                    return rand_point
...
def randon_resampling(main_obj, map_obj, prev_weights):
    percetage_random = 0.05
    if(np.sum(main_obj.weights)/(1+np.sum(prev_weights))<1/2.0):
        percetage_random = 0.95
...
def main():
    ...
    particles = main_obj.generate_random_particle_initial(map_obj, False)
    ...
    while not rospy.is_shutdown():
        try:
            ...
            if(_update):
                ...
                if(is_update):
                        randon_resampling(main_obj, map_obj, prev_weights)
                ...
                show_interval += 1
                if show_interval  == 3:
                    main_obj.particles = main_obj.generate_random_particle_kidnapped(map_obj, False)
                prev_weights = main_obj.weights
